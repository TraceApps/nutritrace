/**
 * /api/v1/profile — the user's gender and date of birth.
 * Same xCore() the MCP get_profile tool calls. See diary.js in this same
...
import { getProfileCore } from '../../../lib/mcp/tools/profile.js';
...
router.get('/', requireScope('mcp:read'), wrap((req, res) => {
  try {
    res.json(getProfileCore(req.apiUser.id));
  } catch (e) {
