...
export async function fetchSleepForDate(accessToken, dateStr) {
  ...
  // Sleep Quality sub-metrics: time to sound sleep, interruptions, full
  // awakenings, restlessness and sound sleep. The walk and its calibration
  // notes live in src/lib/sleep-quality.js, shared with Health Connect.
  ...
  // which is the shape the walk reads.
  const googleMinutesAsleep = parseInt(summary.minutesAsleep, 10) || null;

  const stageSegs = best.stages || [];
  const { interruptionsMin, fullAwakenings, restlessnessMin, timeToSoundSleep, soundSleepMin } =
    deriveSleepQuality(stageSegs);
