 ...
 * GET /api/v1/meals/:id. Throws a plain Error on a missing meal.
 */
export function getMealDetailsCore(userId, { meal_id } = {}) {
  ...
  if (!row) throw new Error(`meal_id ${meal_id} not found in your catalog.`);
...
export function registerGetMealDetails(server, { userId }) {
  server.registerTool(
    ...
    async ({ meal_id }) => {
      try {
        return toolResult(getMealDetailsCore(userId, { meal_id }));
      } catch (e) {
