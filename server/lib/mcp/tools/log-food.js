 ...
 * POST /api/v1/diary/:date/food. Throws a plain Error on bad input or a
 * tombstoned day so callers on either side can decide how to surface it.
 */
export function logFoodCore(userId, { food_id, date, meal, quantity, portion, unit, notes } = {}) {
  ...
  if (!food) throw new Error(`food_id ${food_id} not found in your catalog.`);
  ...
  if (Number.isFinite(portion) && hasAltUnits) {
    throw new Error(
      ...
      "wouldn't match what the app UI produces. Omit the portion argument (uses 1x " +
  ...
  if (Number.isFinite(portion) && !foodPortion) {
    throw new Error(
      ...
      "can't be scaled correctly. Log without the portion argument (uses 1x the food's " +
  ...
  try {
    dispatchWebhookEvent(userId, 'meal.logged', { date: day, items: [item] });
...
export function registerLogFood(server, { userId }) {
  server.registerTool(
    ...
    async ({ food_id, date, meal, quantity, portion, unit, notes }) => {
      try {
        return toolResult(logFoodCore(userId, { food_id, date, meal, quantity, portion, unit, notes }));
      } catch (e) {
