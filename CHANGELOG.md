...
- **CookTrace pull: bulk-import every recipe in one action.** Settings, Connected Services, CookTrace, Import All Recipes pages through every recipe on the CookTrace side and upserts each one via the same `(source_app, source_external_id)` dedup key the single-recipe picker uses. Live progress on the button ("3 / 42"), per-recipe failures accumulate as a skip count rather than aborting the batch, and re-runs update existing imports in place. Confirmation prompt shows the count before it starts so a stray tap does not kick off 100+ round trips.
- **CookTrace pull: bulk-import the pantry into NutriTrace's foods library.** Companion to the recipe bulk import. Settings, Connected Services, CookTrace, Import Pantry Items pulls every leaf pantry row from CookTrace and upserts it into NT's foods library. Generic parents with variants are deliberately skipped: their variants carry the real nutrition, so a top-level "Flour" next to "Flour, Bread" and "Flour, All-Purpose" would be misleading. Variant rows land as "Parent, Child" (e.g. "Flour, Bread") so they read cleanly in the foods list. Requires the new `read:pantry` scope on the CookTrace token. Nutrition uses the same 4-level resolver the recipe pull uses (variant own, variant fallback to parent's designated variant, generic own, generic fallback), and calories derive from carbs / protein / fat via Atwater when the pantry row only stored macros. Foods table gained `source_app` / `source_external_id` / `source_url` columns and a partial unique index on `(user_id, source_app, source_external_id)` for the upsert. Server route `POST /api/foods` now upserts on those columns; behavior for non-federated foods is unchanged.
- **CookTrace pantry as a searchable Foods source.** The Foods tab's source chip row gains a **CookTrace** chip next to Open Food Facts / USDA / Mealie, backed by the new `q` / `limit` / `offset` params on CookTrace's `/api/v1/pantry`. Search-and-pick a single pantry row instead of backfilling the whole pantry; the picked row opens NT's Food editor pre-filled and carries the same `(source_app, source_external_id)` stamp the bulk import uses, so saving updates that row rather than duplicating it. Participates in All mode alongside the other sources, with per-source counts and infinite scroll. Same leaf-only rule as the bulk import (generic parents with variants are filtered server-side), and search matches the composed "Flour, Bread" display name so a variant stays findable by its generic's word. Also fills in the `.source-badge-cooktrace` / `.asc-cooktrace` colours, which were missing entirely, so the existing Recipes-tab CookTrace chip stops rendering an uncoloured dot.
- **CookTrace pull: source-deleted indicator on the Meal Editor.** When the background refresh probe comes back with a hard 404 (the CookTrace recipe was deleted or the token lost read access to it), NT now shows a warning-tinted "CookTrace source no longer available" banner in place of the refresh prompt, with **Unlink** (keep the recipe, drop the CookTrace provenance) and **Delete** (drop the NT copy) actions. Network failures and other non-404 errors do NOT trigger the banner (offline users would otherwise see it constantly); only an unambiguous "gone" from CookTrace does.
- **CookTrace pull: auto-refresh prompt when the source recipe changes.** Opening a CT-imported recipe in NT's Meal Editor now quietly checks the CookTrace source in the background; when CT's `updated_at` is newer than the local NT copy, a "CookTrace has newer content" banner appears with Refresh + Dismiss buttons. Refresh remaps ingredients + totals from CT and applies them to the current row for the next save. Silent on any failure (offline, missing token) and skipped when CookTrace federation is disabled. Never overwrites without user consent. Companion pull-side fixes in the same cycle: server derives calories from macros when a pantry entry only has carbs / protein / fat, follows variant chains in both directions (generic points at designated variant OR variant with empty own-nutrition falls back to generic), and adds name-match fallback so unlinked ingredients still get nutrition from a same-named pantry row.
- **CookTrace pull: recipes with a source rollup no longer lose those totals on save.** The Meal Editor recomputes totals from ingredients as the user edits, and it was writing that recompute back onto CT-sourced meals every save. That silently drifted the totals away from what CT's own density-aware Recompute engine had produced (CT can convert "2 cups flour" to grams via `g_per_cup`; MealEditor cannot on the NT side). Save now trusts the source rollup when the meal carries a `source_app` and the items count matches the server baseline (i.e. the user did not add / remove ingredients); a manual ingredient edit or a Recompute-from-Foods still writes the fresh totals as before. Nutrition ingested from CT is also multiplied through by servings on import so it matches NT's whole-recipe convention (previously landed as per-serving values that then re-divided on save).
- **Full-backup restore now carries CookTrace meal provenance and diary completion state.** The meals import lost `source_app` / `source_external_id` / `source_url` / `import_warnings` on restore (the "From CookTrace" badge and back-link would vanish); the diary import lost `completed_at` / `completed_meals`. Both fixed. Cross-device sync (native `dbUpsertFromServer`) now carries the same columns so a pulled meal keeps its badge on the receiving device.
...
- **CookTrace / Mealie images now come through when the source is on your LAN.** The image self-hosting step has an SSRF guard that refuses to fetch from any private or loopback host (10.x, 172.16.x, 192.168.x, 127.x, link-local, IPv6 ULA). That was silently blocking every image on a home-server setup where both NutriTrace and CookTrace / Mealie sit on the same LAN: the raw private-IP URL got stored on the row instead, and any client whose network could not reach that IP saw an empty image slot. NT now trusts the user's own saved `cooktraceBaseUrl` / `mealieBaseUrl` for image fetches only (the user proved they can reach the host by configuring it in Settings), so LAN federation setups actually self-host the thumbnails. Truly cross-network setups (NT on a public origin, CookTrace on a LAN address) still cannot fetch the image, since the NT server itself has no route to the private address; the recipe / pantry row still imports fine without the thumbnail.
- **Bulk pantry import no longer reports imports that did not happen.** `POST /api/foods` has a barcode dedup gate that fires before the insert and returns the pre-existing row untouched, so a CookTrace pantry row whose barcode matches a food you created yourself was deliberately not applied. The bulk importer built its "already seen" set from `source_app='cooktrace'` rows only, so those collisions counted as `imported` and the summary claimed work that never occurred. The importer now reads the returned row back (our own insert and the federation upsert both echo the `source_external_id` that was sent; the barcode gate returns a different row) and reports them separately as "N already in your library (matched by barcode)". Behavior is unchanged, only the count is now truthful: your hand-curated food is still left alone rather than being overwritten by the CookTrace copy. The single-pick path already surfaced this via the Food editor's existing duplicate-barcode warning and its Open Existing button.
