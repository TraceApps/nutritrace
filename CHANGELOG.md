...
- **Food logged through MCP, the API or a diet import no longer appears twice after you save the day in the app** ([#239](https://github.com/TraceApps/nutritrace/issues/239)). Those entries were stored without the id the app's save matches on, so saving the day (even just logging a weight) kept a second copy of each, and a phone with an older copy could make a third. They also could not be deleted from the app. Entries already stored are repaired when the server starts, and copies a phone still holds are recognized instead of added again. Restoring an old export had the same problem and is fixed too. Thanks @Chrristin for the report and the fix it builds on.
- **Open Food Facts products with no name in their main language are found** ([#238](https://github.com/TraceApps/nutritrace/issues/238)). A product whose main-language name was blank was reported as not in Open Food Facts, and left out of name searches, even when it had a name in other languages. NutriTrace now falls back to the other languages, then the generic name. If you picked a language other than English under Open Food Facts in Settings, a product's name in that language is shown when it has one. Thanks @meggiman for the report.
- **Changing a diary entry from a household unit to grams converts instead of multiplying** ([#237](https://github.com/TraceApps/nutritrace/issues/237)). A bread logged as 1 slice (54.7 g) and changed to 66 g showed about 8,700 kcal instead of 159, because the diary read the food's units in a form the converter could not use. Entries already saved on Android are fixed too. Thanks @herver1971 for the report, the root cause and the test.
- **Health Connect counts every part of a night's sleep, and its sleep stages show up** ([#236](https://github.com/TraceApps/nutritrace/issues/236)). Samsung Health splits a night wherever you woke up, and NutriTrace kept only the last piece: a night of 00:46-05:00 and 05:45-06:53 read as 68 minutes. Every session of the night counts now, the same night recorded by two apps is counted once, and an afternoon nap no longer replaces it. Syncing from the app never recognized a sleep stage, so Deep, Light, REM and the Sleep Quality cards stayed empty until the hourly background sync filled some of them in, and a few were stored under names no card reads. Both syncs now share one set of rules and the same names Fitbit and Google Health use, so they give the same numbers from the same records. Thanks @kgenerozov for the report and the code analysis behind it.
