...
- **Daily steps via MCP and REST** ([#219](https://github.com/TraceApps/nutritrace/pull/219)). New `get_steps` MCP tool and `GET /api/v1/steps` route returning persisted daily step observations from wellness data, one row per source, in an inclusive `YYYY-MM-DD` range. Either bound can be omitted to leave that side open; with neither, the range covers the last 90 days. Sources are never merged and missing days are not returned as zeros. Requires the `mcp:read` scope (and `PUBLIC_API_ENABLED=1` for the REST route). Thanks @kgenerozov.
- **Profile facts via MCP and REST** ([#219](https://github.com/TraceApps/nutritrace/pull/219)). New `get_profile` MCP tool and `GET /api/v1/profile` route returning the user's gender and date of birth, as set on the Profile page or during onboarding. Either field is `null` when unset. Requires the `mcp:read` scope (and `PUBLIC_API_ENABLED=1` for the REST route). Both interfaces are backed by the same shared core functions as the rest of the read API. Any `mcp:read` token can read these two fields, so an AI client connected over MCP can see your date of birth. Thanks @kgenerozov.

### Fixed

- **MCP tools and the public API refuse dates that don't exist.** A diary date was only checked for its `YYYY-MM-DD` shape, so `2026-02-31` read back as an empty day, and logging a food, water, a meal or a body stat to it created a diary entry for a date that can't exist. Every MCP tool and `/api/v1` route that takes a diary date now rejects it with an error (`400` over REST). Real dates, leap days included, work as before.
- **Public API date ranges answer `400` when they're wrong, and recent meals takes dates.** `GET /api/v1/steps` answered `200` with an error message in the body for an impossible date or a reversed range; it now returns `400` like the other routes. `GET /api/v1/meals/recent` ignored `start`/`end`, which the matching MCP tool has accepted since [#215](https://github.com/TraceApps/nutritrace/pull/215); it now filters by them too.
