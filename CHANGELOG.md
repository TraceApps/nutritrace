...
- **Read body-composition observations through MCP and the optional Public API.** The new read-only `get_body_composition` tool returns saved scale and body-composition observations over an inclusive date range with source provenance preserved; the same read is available at `/api/v1/body-composition` when the Public API is enabled.
