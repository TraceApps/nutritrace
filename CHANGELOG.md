...
- **Diary's summary bar stays on the tab bar** ([#208](https://github.com/TraceApps/nutritrace/issues/208)). On an iPhone the summary bar could float above the tab bar with a see-through gap and a dark strip below it. The two were positioned separately: the tab bar at the bottom of the screen, and the summary bar at a distance worked out from the tab bar's height plus the iPhone's home-indicator area, so anything that moved one without the other pulled them apart. They now sit together in one container anchored to the bottom of the screen, so the summary bar always rests directly on the tab bar, or directly above the home-indicator area when the tab bar is hidden. Layout on Android and desktop is unchanged. Thanks @zen-pavel for the report.
