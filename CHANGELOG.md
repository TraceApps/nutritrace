...
- **Opt-in trace-level request diagnostics.** `LOG_LEVEL=trace` adds the most verbose server level while request bodies require the separate `TRACE_REQUEST_BODIES=1` switch. Body output is route-limited, recursively credential-redacted, stripped of inline image contents, and size-capped. All write requests now report observed body size, including structured diagnostics for parser-rejected HTTP 413 requests.
