...
- **The web app keeps working without a connection** ([#211](https://github.com/TraceApps/nutritrace/issues/211)). First part: your diary and your own food catalogue are kept in the browser, so in a dead zone the app still opens, the day still adds up, and you can log a food you already have. What you log is held and goes up on its own when the connection comes back, merged by the server exactly as the Android app's changes are, so logging on a phone and a laptop at the same time can't lose an entry. The menu button shows an amber cloud while anything is waiting, and red if your server refuses it. Signing out sends what's waiting first, then clears the copy held in the browser. Sources that need the network say so instead of looking empty: Open Food Facts, USDA, Mealie and CookTrace read "needs a connection" next to their name, a search with no local match explains that only your own foods were searched, and a barcode scanned offline says only your own foods were checked rather than calling the product unknown. Still needs a connection for now: those searches and lookups, photos, the wellness providers, Trace and anything admin. Deliberately not built on Background Sync, which Safari doesn't have, so this works the same on iPhone. The installed app also survives a reload or a relaunch with no connection: it now keeps its own code in the browser, where before it only kept a fallback page and came back blank. Food photos you've seen are kept too, so the diary and your food list don't fall back to blank squares offline. A photo that isn't there (not yet cached, or removed on the server) shows the same placeholder as an entry with no picture, instead of a broken-image icon. Manual activity works offline too: log a walk in a dead zone and it appears on the day at once, with the day's calories-out recalculated from what you have. The wearable half of that number is whatever your provider last sent, since those need a connection of their own. You can also add, edit and delete your own foods, meals and recipes offline: a food made in a dead zone appears in your list straight away, can be logged immediately, and when the connection returns it and everything logged from it go up together, with the diary entries pointing at the real food rather than a placeholder. Settings and goals change offline too, so a switch flicked in a dead zone stays flicked and goes up with everything else. The fasting timer works offline as well: start a fast with no connection, reload the app and it is still counting, and ending it offline records the fast with the right start and end once you are back. Only one fast runs at a time offline, exactly as the server insists. Anything created offline stays editable the moment its queue goes up, where a change made in that instant could previously have arrived as a second copy. What the browser keeps is what the server last sent, so a food deleted on your phone stays deleted here rather than reappearing the moment the connection drops. Your wellness figures stay on screen offline too: the day you have already looked at keeps its steps, sleep and heart numbers instead of emptying, and a day never opened says it needs a connection. The copy the browser keeps also follows you: what was read before the app knew who was signed in is brought across rather than left in a corner nothing reads. Signing out sends what is waiting first, and if it cannot reach your server it asks before discarding it rather than clearing it silently. Your profile works offline too, picture included: choose one in a dead zone and it travels with the profile, becoming an ordinary file on your server when the queue goes up. If your server refuses what is waiting, it says so in words and keeps trying, rather than showing a red cloud with no explanation, and the reason is written to Settings, Diagnostics.
