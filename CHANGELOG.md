...
- **Cronometer import points you to the right export, and keeps evening times and supplements.** Cronometer's per-food export is the **Export Food & Recipe Entries** button (it downloads `servings.csv`), but NutriTrace's instructions still called it "Servings", so people reasonably picked **Export Daily Nutrition** instead and got a generic error. The instructions now name the right button, and uploading the Daily Nutrition file explains what it is and which export to use. That check reads the file's columns, so a renamed file gets the same help. Two bugs found by running a real Cronometer export through the importer are fixed as well: Cronometer writes 12-hour times ("7:45 PM") and the AM/PM was being dropped, so every evening entry was saved as morning; and entries Cronometer exports with blank calories, such as supplements and most spices, were skipped along with their vitamins and minerals. ZIP detection now reads the file's contents instead of trusting its name.
