...
- **Completion badges on the Date Picker** (#207 companion). Tap the current date to jump; the calendar cells for days you closed carry a small green corner dot so month-view scanning is as informative as the week strip.
- **Close today from the bedtime notification** (Android, #207 companion). When the bedtime reminder fires and today is not already marked, it carries a **Close today** quick action that marks the day complete in one tap without opening the app. Suppressed when today is already closed.
- **Weekly summary shows adherence** (#207 companion). The weekly summary push notification and email both include a "Marked complete: X of 7 days" line when the user marked at least one day in the window.
- **Statistics adherence surfaces** (#207 companion). Right rail gains a "Marked complete" KPI showing N of M days and a percentage over the viewed range; the chart itself grows a row of tiny green dots below the x-axis marking which days were closed. Both work for every metric (steps, weight, nutrients, etc.) since completion is a property of the diary row, not of the metric.
- **Per-meal completion, opt-in** (#207 companion). Off by default. Turn on under Settings, Diary, Show Meal Completion to add a check toggle to each meal card header. When on, marking the day complete checks for empty-and-unmarked slots and confirms before closing; confirming records the intentional skip so a repeated pattern (e.g. fasters skipping breakfast) doesn't re-prompt. Sync across devices uses union-merge so a slot marked on either device stays marked. Storage is a JSON array of slot indexes on the diary row; the column ships regardless of the setting so devices with different toggles can sync cleanly.
