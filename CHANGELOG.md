...
First dev pre-release of the 1.4.0 minor. The headline is that the web app keeps working without a connection: your diary and your own foods are kept in the browser, what you log while offline goes up on its own, and the installed app survives a relaunch in a dead zone. Also in: body composition through MCP and the REST API, update checks that are off until you ask for them, and fonts served by your own instance instead of Google.


### Added

- **Body composition through MCP and the REST API** ([#234](https://github.com/TraceApps/nutritrace/pull/234)). A new `get_body_composition` read tool returns the scale readings saved from your connected sources (weight, body fat, muscle, water, and the per-limb figures some scales report) over a date range. Each source is kept separate rather than merged, so you can see what each device reported. The same read is at `/api/v1/body-composition` when the Public API is on. Contributed by @kgenerozov.


- **The web app keeps working without a connection** ([#211](https://github.com/TraceApps/nutritrace/issues/211)). Your diary and your own food catalogue are kept in the browser, so in a dead zone the app still opens, the day still adds up, and you can log a food you already have. What you log is held and goes up on its own when the connection comes back, merged by the server exactly as the Android app's changes are, so logging on a phone and a laptop at the same time can't lose an entry. The menu button shows an amber cloud while anything is waiting, and red if your server refuses it. Signing out sends what's waiting first, and if it cannot reach your server it asks before discarding it rather than clearing it silently. Deliberately not built on Background Sync, which Safari doesn't have, so this works the same on iPhone. The installed app also survives a reload or a relaunch with no connection: it now keeps its own code in the browser, where before it only kept a fallback page and came back blank. Food photos you've seen are kept too, so the diary and your food list don't fall back to blank squares offline.

- **More than the diary works offline** ([#211](https://github.com/TraceApps/nutritrace/issues/211)). You can add, edit and delete your own foods, meals and recipes: a food made in a dead zone appears in your list straight away, can be logged immediately, and when the connection returns it and everything logged from it go up together, with the diary entries pointing at the real food rather than a placeholder. Manual activity works too: log a walk and it appears on the day at once, with the day's calories-out recalculated from what you have, the wearable half being whatever your provider last sent. Settings and goals change offline, so a switch flicked in a dead zone stays flicked and goes up with everything else. The fasting timer keeps counting through a reload, and ending a fast offline records it with the right start and end once you are back, with only one fast running at a time exactly as the server insists. Your profile works offline as well, picture included: choose one in a dead zone and it travels with the profile, becoming an ordinary file on your server when the queue goes up. Your wellness figures stay on screen: a day you have already looked at keeps its steps, sleep and heart numbers instead of emptying. Anything created offline stays editable the moment its queue goes up, where a change made in that instant could previously have arrived as a second copy, and what the browser keeps is what the server last sent, so a food deleted on your phone stays deleted here rather than reappearing the moment the connection drops.

- **What still needs a connection says so** ([#211](https://github.com/TraceApps/nutritrace/issues/211)). Open Food Facts, USDA, Mealie and CookTrace read "needs a connection" next to their name, a search with no local match explains that only your own foods were searched, and a barcode scanned offline says only your own foods were checked rather than calling the product unknown. A wellness day you have never opened says it needs a connection, and a photo that isn't there (not yet cached, or removed on the server) shows the same placeholder as an entry with no picture instead of a broken-image icon. If your server refuses what is waiting, it says so in words and keeps trying, and the reason is written to Settings, Diagnostics. Still needs a connection for now: those searches and lookups, photos, the wellness providers, Trace and anything admin.
...
- **The Copy sheet's buttons no longer sit under Android's navigation bar** ([#233](https://github.com/TraceApps/nutritrace/issues/233)). Copying a diary item to another day put Cancel and Copy underneath the 3-button navigation bar, because that one sheet overwrote the spacing that keeps content clear of it. Thanks @nomad64 for the report.
- **The Trace button no longer covers what's on top of it** ([#233](https://github.com/TraceApps/nutritrace/issues/233)). It floated above every sheet and dialog, so wherever you had dragged it, it could sit over a title or a button. It now stays above the page but underneath anything opened on top of it, and it still moves where you put it. Thanks @nomad64 for the report.
...
- **Work queued while a sync was running no longer waits for you to do something else** before it goes up, and a change that could not be sent because the page had not built its connection yet now carries the token your server asks for, instead of being refused over and over.
...
- **A profile saved before the app had finished checking your server no longer goes to the wrong place.** The Profile screen decided once, as it opened, whether this was a single-user instance, and if the check had not answered yet it wrote your name and picture to local settings instead of your account. It now follows the answer whenever it arrives.
