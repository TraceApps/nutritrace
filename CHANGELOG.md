...
- **CookTrace pull: auto-refresh prompt when the source recipe changes.** Opening a CT-imported recipe in NT's Meal Editor now quietly checks the CookTrace source in the background; when CT's `updated_at` is newer than the local NT copy, a "CookTrace has newer content" banner appears with Refresh + Dismiss buttons. Refresh remaps ingredients + totals from CT and applies them to the current row for the next save. Silent on any failure (offline, missing token) and skipped when CookTrace federation is disabled. Never overwrites without user consent. Ingredient nutrition resolves through CookTrace's pantry the way you would expect: a generic with a designated variant takes that variant's values, a variant with no values of its own inherits from its generic, calories are derived from carbohydrate, protein and fat when a pantry entry only recorded macros, and an ingredient with no explicit pantry link still matches a same-named pantry row. Each ingredient also carries its pantry photo.
- **Imported recipes keep CookTrace's own nutrition totals.** CookTrace's rollup engine does density-aware unit conversion (it knows "2 cups flour" in grams via `g_per_cup`), which NutriTrace cannot reproduce from the ingredient rows alone. An imported recipe therefore keeps the totals CookTrace computed, and saving in the Meal Editor preserves them rather than substituting a weaker items-derived sum. Editing the ingredients yourself, or running Recompute, hands control back to NutriTrace's own calculation. Nutrition arrives as whole-recipe values scaled to your serving count, so the per-serving figures match what CookTrace shows.
- **Backup and cross-device sync cover the new federation and completion data.** The CookTrace provenance on an imported recipe (its source link, the From CookTrace badge, any import warnings) and the day / per-meal completion marks all survive a full-backup export and restore, and propagate correctly to a second device through the normal sync path.
